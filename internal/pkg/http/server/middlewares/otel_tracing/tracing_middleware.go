...
func HttpTrace(opts ...Option) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			request := c.Request()
			// doesn't contain trace information and after completing trace on new ctx
			// we should go back to our old savedCtx
			savedCtx := request.Context()

			defer func() {
				// we should go back to previous context in end of our operation because
				// new context contains child spans, and if we don't set it back to
				// previous context, after returning from this method all further parent
				// spans becomes a new child for existing child span!
			...
			// add the new context into the request, because new ctx contains our
			// created span and we want all inner spans in next middlewares become
			// child span of this span pass the span through the request context
